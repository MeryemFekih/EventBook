// seed.js — creates test accounts and sample events (VULNERABLE VERSION)
// NOTE: passwords stored in PLAIN TEXT here on purpose (weak auth vulnerability).
const db = require('./db');

db.exec('DELETE FROM notifications; DELETE FROM reservations; DELETE FROM events; DELETE FROM users;');

const insertUser = db.prepare('INSERT INTO users (username, password, role) VALUES (?, ?, ?)');
insertUser.run('alice', 'alice123', 'user'); // will own an event
insertUser.run('bob', 'bob123', 'user');     // visitor
